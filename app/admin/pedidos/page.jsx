...
export default function AdminPedidos() {
  ...
  const [confirm, setConfirm] = useState(null); // { pedido, nuevoEstado }
    ...
    <div style={s.page}>
      ...
      <main style={s.main}>
        ...
        <div style={s.statsGrid}>
          {[
            { icon: "🛒", label: "Total",      value: contadores.total,     color: "#9a031e"  },
            { icon: "⏳", label: "Pendientes", value: contadores.pendiente, color: "#f59e0b"  },
            { icon: "💳", label: "Pagados",    value: contadores.pagado,    color: "#3b82f6"  },
            { icon: "🚚", label: "Enviados",   value: contadores.enviado,   color: "#10b981"  },
            { icon: "❌", label: "Cancelados", value: contadores.cancelado, color: "#ef4444"  },
          ].map(st => (
            <div key={st.label} style={{ ...s.statCard, borderLeftColor: st.color, cursor: "pointer" }}
              onClick={() => setFiltro(st.label === "Total" ? "todos" : st.label.toLowerCase().replace("ados","ado").replace("entes","ente"))}>
        ...
        <div style={s.filtros}>
          {["todos", "pendiente", "pagado", "enviado", "cancelado"].map(f => (
            <button key={f} onClick={() => setFiltro(f)} style={{
              ...
              {f === "todos" ? `Todos (${contadores.total})` : `${f.charAt(0).toUpperCase() + f.slice(1)} (${contadores[f] ?? 0})`}
            </button>
          ))}
        </div>
          ...
          {loading ? (
            ...
            <div style={s.tableWrapper}>
              <table style={s.table}>
                ...
                <tbody>
                  {pedidos.length === 0 ? (
                    <tr><td colSpan={6} style={{ ...s.td, textAlign: "center", color: "#a0a0a0", padding: 40 }}>
                      ...
                      <tr key={p.id_pedido} style={{ background: i % 2 === 0 ? "rgba(154,3,30,0.04)" : "transparent" }}>
                        ...
                        <td style={s.td}>
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                            {/* Ver detalle */}
                            <button onClick={() => verDetalle(p.id_pedido)} style={s.btnAccion}>
                              👁️
                            </button>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
                    ...
                    <div style={s.infoBox}>
                      ...
                      <p style={s.infoRow}><b>Estado:</b>{" "}
                        <span style={{ ...s.badge, background: ESTADO_STYLE[detalle.pedido.estado]?.bg, color: ESTADO_STYLE[detalle.pedido.estado]?.color, border: `1px solid ${ESTADO_STYLE[detalle.pedido.estado]?.border}` }}>
                          {ESTADO_STYLE[detalle.pedido.estado]?.label}
                        </span>
                      </p>
                      <p style={s.infoRow}><b>Total:</b> <span style={{ color: "#10b981", fontWeight: 700 }}>${detalle.pedido.total.toFixed(2)}</span></p>
                    </div>
                  </div>

                  {/* Productos */}
                  <p style={{ color: "#a0a0a0", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>🛍️ Productos</p>
                  <table style={{ ...s.table, fontSize: 13 }}>
                    ...
                    <tbody>
                      {detalle.productos.length === 0 ? (
                        <tr><td colSpan={4} style={{ ...s.td, textAlign: "center", color: "#a0a0a0" }}>Sin productos registrados</td></tr>
                      ) : detalle.productos.map((prod, i) => (
                        <tr key={i} style={{ background: i % 2 === 0 ? "rgba(154,3,30,0.04)" : "transparent" }}>
                          <td style={s.td}>{prod.nombre}</td>
                          <td style={s.td}>{prod.cantidad}</td>
                          <td style={s.td}>${prod.precio_unit.toFixed(2)}</td>
                          <td style={{ ...s.td, fontWeight: 700, color: "#10b981" }}>${prod.subtotal.toFixed(2)}</td>
                        </tr>
                      ))}
                      <tr>
                        <td colSpan={3} style={{ ...s.td, textAlign: "right", fontWeight: 700 }}>Total:</td>
                        <td style={{ ...s.td, fontWeight: 700, color: "#10b981", fontSize: 16 }}>${detalle.pedido.total.toFixed(2)}</td>
                      </tr>
                    </tbody>
                  </table>
                </>
          ...
          <div style={{ ...s.modalBox, maxWidth: 400 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ ...s.modalBody, textAlign: "center", padding: 24 }}>
              <p style={{ color: "#d9d9d9", fontSize: 15 }}>
                ¿Cambiar el pedido <strong style={{ color: "#c1121f" }}>#{confirm.pedido.id_pedido}</strong> de{" "}
            ...
            <div style={s.modalFooter}>
              ...
              <button onClick={confirmarCambio} disabled={!!procesando} style={{ ...s.btnPrimary, opacity: procesando ? 0.6 : 1 }}>
