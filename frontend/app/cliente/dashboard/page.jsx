  ...
  useEffect(() => {
    ...
    fetch(`${API}/cliente/dashboard`, { headers:{ Authorization:`Bearer ${t}` } })
      ...
      .then(d  => {
        setData(d);
        setLoad(false);
        fetch(`${API}/cliente/recomendaciones`, { headers:{ Authorization:`Bearer ${t}` } })
          .then(r => r.json())
          .then(rd => { if (rd.recomendaciones) setRecomendaciones(rd.recomendaciones); })
          .catch(() => {});
      })
  ...
  return (
    ...
    <div className={`vmb-root ${theme}`}>
      ...
      <main className="vmb-main">
        ...
        {/* ── ML Recommendations grouped by category ────────────── */}
        {recomendaciones.length > 0 && (
          <section className="vmb-section">
            <div className="vmb-section__head">
              <h2 className="vmb-section__title">
                Según tus <em>compras anteriores</em>
              </h2>
              <a href="/cliente/tienda" className="vmb-section__link">Ver todos →</a>
            </div>
            {(() => {
              const grouped = {};
              ...
              const catLabels = { electronico: "Electrónico", ropa: "Ropa", hogar: "Hogar", deportes: "Deportes", otros: "Otros" };
              return Object.entries(grouped).map(([cat, prods]) => (
                <div key={cat} style={{ marginBottom: 24 }}>
                  <div style={{ fontFamily: "var(--font-c)", fontSize: 13, fontWeight: 600, letterSpacing: 3, textTransform: "uppercase", color: "var(--text-2)", marginBottom: 12, paddingLeft: 4 }}>
                    {catLabels[cat] || cat}
                  </div>
