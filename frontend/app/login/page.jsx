...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("http://localhost:8000/auth/login/auto", {
    method: "POST",
    ...
    body: JSON.stringify(form)
      ...
      if (guest.length > 0) {
        for (const item of guest) {
          try {
            const body = item.plataforma
              ? { tipo:"externo", id_producto_externo:item.id, nombre:item.nombre, precio:item.precio, peso:item.peso||.5, categoria:item.categoria||"electronico", plataforma:item.plataforma, cantidad:item.qty||1 }
              : { tipo:"local", id_producto:item.id, cantidad:item.qty||1 };
            await fetch("http://localhost:8000/cliente/carrito/agregar", {
    ...
    <>
      ...
      <div className="lp-root">
        ...
        <a href="/" style={{
          ...
          transition: "all 0.2s",
        }} onMouseOver={e => { e.currentTarget.style.color = "#c1121f"; e.currentTarget.style.borderColor = "rgba(193,18,31,0.7)"; }}
           onMouseOut={e => { e.currentTarget.style.color = "#7a7570"; e.currentTarget.style.borderColor = "rgba(154,3,30,0.3)"; }}>
